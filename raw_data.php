...
global $noIngressPortals;
if (!$noIngressPortals ) {
    if ($d["lastportals"] == "true") {
        if ($lastportals != "true") {
            $d["portals"] = $scanner->get_ingress_portals($swLat, $swLng, $neLat, $neLng);
        } else {
            if ($newarea) {
                $d["portals"] = $scanner->get_ingress_portals($swLat, $swLng, $neLat, $neLng, 0, $oSwLat, $oSwLng, $oNeLat, $oNeLng);
            } else {
                $d["portals"] = $scanner->get_ingress_portals($swLat, $swLng, $neLat, $neLng, time());
            }
        }
    }
}
